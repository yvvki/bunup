            ...
            for (let i = 0; i < l; i++) {
                  ...
                  if (node) {
                        if (isText && !optimized) {
                              if (
                                    i + 1 < l &&
                                    normalizeVNode(children[i + 1]).type ===
                                          Text
                              ) {
                                    insert(
                                          createText(
      ...
      return defineComponent({
            ...
            __asyncHydrate(el, instance, hydrate2) {
                  const doHydrate = hydrateStrategy
                        ? () => {
                                const teardown = hydrateStrategy(hydrate2, cb =>
                                      forEachElement(el, cb),
                                );
                                if (teardown) {
                                      (
                                            instance.bum || (instance.bum = [])
                                      ).push(teardown);
                                }
                          }
                        : hydrate2;
      ...
      const setupRenderEffect = (
            ...
            const componentUpdateFn = () => {
                  if (!instance.isMounted) {
                        ...
                        if (
                              (vnodeHook =
                                    next.props && next.props.onVnodeUpdated)
                        ) {
                              queuePostRenderEffect(
                                    () =>
                                          invokeVNodeHook(
      ...
      const patchKeyedChildren = (
            ...
            if (i > e1) {
                  if (i <= e2) {
                        const nextPos = e2 + 1;
                        const anchor =
                              nextPos < l2 ? c2[nextPos].el : parentAnchor;
                        while (i <= e2) {
                              patch(
                                    null,
                                    (c2[i] = optimized
...
function doWatch(source, cb, options = EMPTY_OBJ) {
      ...
      if (isInSSRComponentSetup) {
            if (flush === 'sync') {
                  ...
                  ssrCleanup =
                        ctx.__watcherHandles || (ctx.__watcherHandles = []);
            ...
            registerDep(instance, setupRenderEffect, optimized2) {
                  ...
                  instance.asyncDep
                        ...
                        .then(asyncSetupResult => {
                              ...
                              const placeholder =
                                    !hydratedEl && instance.subTree.el;
                              setupRenderEffect(
                                    instance,
                                    vnode2,
...
function initCustomFormatter() {
      ...
      function formatInstance(instance) {
            ...
            if (instance.setupState !== EMPTY_OBJ) {
                  blocks.push(
                        createInstanceBlock('setup', instance.setupState),
                  );
            }
            if (instance.data !== EMPTY_OBJ) {
                  blocks.push(
                        createInstanceBlock('data', toRaw(instance.data)),
                  );
            }
                        ...
                        if (owner) {
                              if (owner.__hmrId) {
                                    if (!this._childStyles)
                                          this._childStyles =
                                                /* @__PURE__ */ new Map();
                                    let entry = this._childStyles.get(
                                          owner.__hmrId,
                                    );
                                    if (!entry) {
            ...
            if (isMultiple) {
                  if (isArrayValue) {
                        ...
                        if (
                              ...
                              option.selected =
                                    looseIndexOf(value, optionValue) > -1;
                        }
                  } else {
