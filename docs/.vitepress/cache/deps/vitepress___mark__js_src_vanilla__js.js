...
var DOMIterator = class _DOMIterator {
        ...
        onIframeReady(ifr, successFn, errorFn) {
                try {
                        if (
                                ifr.contentWindow.document.readyState ===
                                'complete'
                        ) {
                                if (this.isIframeBlank(ifr)) {
        ...
        compareNodeIframe(node, prevNode, ifr) {
                ...
                if (compCurr & prev) {
                        if (prevNode !== null) {
                                const compPrev =
                                                prevNode.compareDocumentPosition(
                                                        ifr,
                                                ),
                                        after =
...
var Mark = class {
        ...
        callNoMatchOnInvalidRanges(range, last) {
                ...
                if (range && typeof range.start !== 'undefined') {
                        ...
                        this.log(
                                `Ignoring invalid range: ${JSON.stringify(range)}`,
                        );
                        this.opt.noMatch(range);
                }
                return {
        ...
        mark(sv, opt) {
                ...
                let totalMatches = 0,
                        fn = 'wrapMatches';
                const {keywords: kwArr, length: kwArrLen} =
                        ...
                        handler = kw => {
                                let regex = new RegExp(
                                                this.createRegExp(kw),
                                                `gm${sens}`,
                                        ),
                                        matches = 0;
                                this.log(
                                        `Searching with expression "${regex}"`,
                                );
                                this[fn](
                                        regex,
                                        1,
        ...
        unmark(opt) {
                ...
                this.iterator.forEachNode(
                        ...
                        node => {
                                const matchesSel = DOMIterator.matches(
                                                node,
                                                sel,
                                        ),
                                        matchesExclude =
                                                this.matchesExclude(node);
