                ...
                if (key === LEAF) {
                        ...
                        for (let pos = 0; pos < key.length; ++pos, ++i) {
                                ...
                                for (let j = jmin; j < jmax; ++j) {
                                        ...
                                        const rpl =
                                                matrix[prevRowOffset + j] +
                                                +different;
                                        const del =
                                                matrix[prevRowOffset + j + 1] +
                                                1;
                                        const ins =
                                                matrix[thisRowOffset + j] + 1;
                                        const dist = (matrix[
                                                thisRowOffset + j + 1
                                        ] = Math.min(rpl, del, ins));
                                        if (dist < minDistance)
                                                minDistance = dist;
                                }
...
var MiniSearch = class _MiniSearch {
        ...
        constructor(options) {
                ...
                const autoVacuum =
                        options.autoVacuum == null ||
                        options.autoVacuum === true
        ...
        async performVacuuming(options, conditions) {
                ...
                if (this.vacuumConditionsMet(conditions)) {
                        const batchSize =
                                options.batchSize ||
                                defaultVacuumOptions.batchSize;
                        const batchWait =
                                options.batchWait ||
                                defaultVacuumOptions.batchWait;
                        let i = 1;
        ...
        vacuumConditionsMet(conditions) {
                ...
                return (
                        this.dirtCount >= minDirtCount &&
                        this.dirtFactor >= minDirtFactor
                );
        }
        /**
        ...
        get dirtFactor() {
                return (
                        this._dirtCount /
                        (1 + this._documentCount + this._dirtCount)
                );
        }
        /**
        ...
        autoSuggest(queryString, options = {}) {
                ...
                for (const {score, terms} of this.search(
                        queryString,
                        options,
                )) {
        ...
        static getDefault(optionName) {
                if (defaultOptions.hasOwnProperty(optionName)) {
                        ...
                        throw new Error(
                                `MiniSearch: unknown option "${optionName}"`,
                        );
                }
        }
        /**
        ...
        static async loadJSAsync(js, options) {
                ...
                miniSearch._documentIds =
                        await objectToNumericMapAsync(documentIds);
                miniSearch._fieldLength =
                        await objectToNumericMapAsync(fieldLength);
                miniSearch._storedFields =
                        await objectToNumericMapAsync(storedFields);
                for (const [shortId, id] of miniSearch._documentIds) {
                        miniSearch._idToShortId.set(id, shortId);
                }
                let count = 0;
        ...
        executeQuerySpec(query, searchOptions) {
                ...
                const boosts = (options.fields || this._options.fields).reduce(
                        (boosts2, field) => ({
                                ...boosts2,
                                [field]:
                                        getOwnProperty(options.boost, field) ||
                                        1,
                        }),
                        {},
                );
                ...
                if (query.fuzzy) {
                        ...
                        const maxDistance =
                                fuzzy < 1
                                        ...
                                        : fuzzy;
                        if (maxDistance)
                                fuzzyMatches = this._index.fuzzyGet(
                                        query.term,
                                        maxDistance,
                                );
                }
                ...
                if (fuzzyMatches) {
                        for (const term of fuzzyMatches.keys()) {
                                const [data2, distance] =
                                        fuzzyMatches.get(term);
                                if (!distance) {
                                        continue;
                                }
                                const weight =
                                        (fuzzyWeight * term.length) /
                ...
                for (const [shortId, id] of this._documentIds) {
                        const score = options.boostDocument
                                ? options.boostDocument(
                                          id,
                                          '',
                                          this._storedFields.get(shortId),
                                  )
                                : 1;
                        results.set(shortId, {
                                score,
        ...
        combineResults(results, combineWith = OR) {
                ...
                if (!combinator) {
                        throw new Error(
                                `Invalid combination operator: ${combineWith}`,
                        );
                }
        ...
        termResults(
                ...
                for (const field of Object.keys(fieldBoosts)) {
                        ...
                        for (const docId of fieldTermFreqs.keys()) {
                                ...
                                const fieldLength =
                                        this._fieldLength.get(docId)[fieldId];
        ...
        removeFieldLength(documentId, fieldId, count, length) {
                ...
                const totalFieldLength =
                        this._avgFieldLength[fieldId] * count - length;
                this._avgFieldLength[fieldId] = totalFieldLength / (count - 1);
        }
        /**
         * @ignore
