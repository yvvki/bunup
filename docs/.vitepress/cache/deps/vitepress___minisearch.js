            ...
            if (key === LEAF) {
                  ...
                  for (let pos = 0; pos < key.length; ++pos, ++i) {
                        ...
                        for (let j = jmin; j < jmax; ++j) {
                              ...
                              const rpl =
                                    matrix[prevRowOffset + j] + +different;
                              const del = matrix[prevRowOffset + j + 1] + 1;
                              const ins = matrix[thisRowOffset + j] + 1;
                              const dist = (matrix[thisRowOffset + j + 1] =
...
var MiniSearch = class _MiniSearch {
      ...
      get dirtFactor() {
            return (
                  this._dirtCount / (1 + this._documentCount + this._dirtCount)
            );
      }
      /**
            ...
            if (query.fuzzy) {
                  ...
                  const maxDistance =
                        fuzzy < 1
                              ? Math.min(
                                      maxFuzzy,
                                      Math.round(query.term.length * fuzzy),
                                )
                              : fuzzy;
                  if (maxDistance)
...
var calcBM25Score = (
      ...
      return (
            ...
            (d +
                  (termFreq * (k + 1)) /
                        (termFreq +
                              k * (1 - b + (b * fieldLength) / avgFieldLength)))
      );
};
