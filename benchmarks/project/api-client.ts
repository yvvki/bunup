...
export class HttpClient {
      ...
      cancelRequest(url: string): void {
            for (const [
                  requestId,
                  controller,
            ] of this.abortControllers.entries()) {
      ...
      private async fetchRequest<T>(
            ...
            switch (responseType) {
                  ...
                  case 'formdata':
                        responseData =
                              (await response.formData()) as unknown as T;
                        break;
                  case 'json':
                  default:
      ...
      private async handleRetry<T>(
            ...
            const delay: number =
                  ...
                  Math.pow(retryConfig.backoffFactor || 1, retryAttempt);
...
export const createFormState = <T extends Record<string, any>>(
      ...
      const fields = Object.keys(initialValues).reduce(
            (acc, key) => {
                  acc[key as keyof T] = createFormFieldState(
                        initialValues[key],
                  );
                  return acc;
            },
...
export class AuthStore extends Store<AuthState> {
      ...
      async login(credentials: LoginCredentials): Promise<void> {
            ...
            try {
                  const response =
                        await this.httpClient.post<AuthTokenResponse>(
