...
describe('Minify Tests', () => {
	...
	test('should minify simple function exports with JSDoc comments', async () => {
		...
		expect(files[0].dts).toMatchInlineSnapshot(
			`"declare function _(a:number,b:number): number;declare function e(x:number,y:number): number;declare function n(numerator:number,denominator:number): number;export{e as multiply,n as divide,_ as add};"`,
		)
	})
	...
	test('should minify complex types and interfaces with comments', async () => {
		...
		expect(files[0].dts).toMatchInlineSnapshot(
			`"interface _{id:string;name:string;email?:string;readonly createdAt:Date;status:e;}declare enum e {Active=\`active\`,Inactive=\`inactive\`,Pending=\`pending\`,Suspended=\`suspended\`}type t=Omit<_,\`id\`|\`createdAt\`>;type n=Partial<Pick<_,\`name\`|\`email\`|\`status\`>>;interface i<T>{success:boolean;data?:T;error?:string;statusCode:number;}export{e as UserStatus,_ as User,n as UpdateUserRequest,t as CreateUserRequest,i as ApiResponse};"`,
		)
	})
	...
	test('should minify mixed exports with constants, functions, and types', async () => {
		...
		expect(files[0].dts).toMatchInlineSnapshot(
			`"declare const n:string;declare const i:number;declare const d:number;declare enum _ {GET=\`GET\`,POST=\`POST\`,PUT=\`PUT\`,DELETE=\`DELETE\`,PATCH=\`PATCH\`}interface e{timeout?:number;retries?:number;headers?:Record<string,string>;withCredentials?:boolean;}type t<T = any>={status:number;data:T;headers:Record<string,string>;timestamp:Date;};declare function a<T>(endpoint:string,method?:_,data?:any,config?:e): Promise<t<T>>;declare const u:{};export{u as default,a as apiRequest,u as api,e as RequestConfig,d as MAX_RETRIES,_ as HttpMethod,i as DEFAULT_TIMEOUT,t as ApiResponse,n as API_BASE_URL};"`,
		)
	})
	...
	test('should minify multiple files with imports and exports', async () => {
		...
		expect(files[0].dts).toMatchInlineSnapshot(
			`"interface t{id:string;createdAt:Date;updatedAt:Date;}interface _ extends t{name:string;email:string;role:n;}declare enum n {Admin=\`admin\`,_=\`user\`,Moderator=\`moderator\`}interface e extends t{title:string;description:string;price:number;category:string;}declare class i{createUser(userData:Omit<_,keyof t>):Promise<_>;updateUser(id:string,updates:Partial<_>):Promise<_>;deleteUser(id:string):Promise<void>;getUserById(id:string):Promise<_|null>;getUsers(role?:n):Promise<_[]>;}declare class d{createProduct(productData:Omit<e,keyof t>):Promise<e>;updateProduct(id:string,updates:Partial<e>):Promise<e>;deleteProduct(id:string):Promise<void>;getProductById(id:string):Promise<e|null>;getProducts(category?:string):Promise<e[]>;}declare function u(): UserService;declare function r(): ProductService;declare const a:{};export{a as default,u as createUserService,r as createProductService,i as UserService,n as UserRole,i as UserManager,_ as User,d as ProductService,d as ProductManager,e as Product,t as BaseEntity};"`,
		)
	})
	...
	test('should minify with complex re-exports and barrel files', async () => {
		...
		expect(files).toHaveLength(1)
		expect(files[0].dts).toMatchInlineSnapshot(
			`"declare namespace c{export{s as subtract,p as sqrt,l as power,i as multiply,m as divide,n as add,o as PI,w as E}}declare function n(a:number,b:number): number;declare function s(a:number,b:number): number;declare function i(a:number,b:number): number;declare function m(a:number,b:number): number;declare function l(base:number,exponent:number): number;declare function p(value:number): number;declare const o:number;declare const w:number;declare namespace g{export{f as truncate,x as slugify,v as reverse,a as capitalize}}declare function a(str:string): string;declare function v(str:string): string;declare function f(str:string,length:number): string;declare function x(str:string): string;declare namespace h{export{r as unique,y as shuffle,V as groupBy,u as chunk}}declare function u<T>(array:T[],size:number): T[][];declare function r<T>(array:T[]): T[];declare function y<T>(array:T[]): T[];declare function V<T,K extends string|number>(array:T[],keyFn:(item:T)=>K): Record<K,T[]>;declare namespace d{export{N as StringValidator}}declare class N{static isValidEmail(email:string):boolean;static isValidURL(url:string):boolean;static isNotEmpty(str:string):boolean;static hasLength(str:string,min:number,max?:number):boolean;}declare namespace b{export{S as NumberValidator}}declare class S{static isValidNumber(value:any):value is number;static isInRange(value:number,min:number,max:number):boolean;static isPositive(value:number):boolean;static isNegative(value:number):boolean;}declare function E(email:string): boolean;declare function L(value:any): boolean;declare function k(numbers:number[]): number;declare const q:{};export{L as validateNumber,E as validateEmail,r as unique,f as truncate,a as toTitleCase,n as sum,s as subtract,p as sqrt,u as splitArray,x as slugify,y as shuffle,v as reverse,i as product,l as power,i as multiply,V as groupBy,m as divide,q as default,r as deduplicate,u as chunk,a as capitalize,k as calculateSum,n as add,d as StringValidator,d as StringUtils,g as String,o as PI,b as NumberValidator,b as NumberUtils,c as Math,w as E,h as Array};"`,
		)
	})
